...
| `test_mysql_config.h` | Range checks of the numeric `MySQLConfig` setters (`port`, `max_buffer_size`, `async_timeout_ms`, `cancel_timeout_ms`, `max_result_bytes`): an out-of-range value is rejected and the previous one kept. |
...
QUERY`, one `KILL` for repeated calls, and the local fallback when the side connection
cannot connect), the connection pool
