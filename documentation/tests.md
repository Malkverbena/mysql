...
| `test_mysql_config.h` | Range checks of the numeric `MySQLConfig` setters (`port`, `max_buffer_size`, `async_timeout_ms`, `cancel_timeout_ms`, `async_batch_rows`, `max_result_bytes`): an out-of-range value is rejected and the previous one kept. |
...
holds the connection), asynchronous streaming (batches of at least `async_batch_rows` rows,
one batch at a time, `async_close()`, closing or releasing a cursor midway, errors, empty
results, a first batch that arrived before it was asked for, and a multi-statement stream),
asynchronous calls
...
connection after a timeout), cancelling an asynchronous operation (on the server with `KILL
QUERY`, one `KILL` for repeated calls, a step of an asynchronous cursor, and the local
fallback when the side connection cannot connect), the connection pool
...
when that reset fails), failed `begin_transaction()`, multiple resultsets (including streaming
